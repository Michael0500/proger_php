...
| `tests/unit/commands/TdsMergeControllerTest.php` | TDS merge (пакет `tds_status.type='PH_TDS'`, внутри `format_type` CAMT053/MT950/ED211/ED743): маппинг D/C, `is_merged` только без пропусков, один пакет PH_TDS обрабатывает все format_type разом, partial при ненайденном/пустом счёте, `--type`/`--delete-source`, `stmt_ref → statement_number`, `dtl.msg_id → message_id` (только CAMT053), `ph_tds_stmt_dtl.other_id → nostro_entries.other_id`, MT950 fallback `op_type → other_id`, ED `edno/eddate/edauthor`, трассировка `stmt_id/line_no/branch_code`, аудит импорта, chunking, построчная блокировка обработки (`is_processing` пропускает занятую пачку и снимается после прогона). |
...
| `tests/unit/commands/PcrControllerTest.php` | PCRFIHIST export (`pcr/export`): формат строк 60/61 — фиксированные ширины и паддинг, разделитель `|`, RUB→RUR, дата баланса `dd/MM/YYYY`, суммы с `.`, `operationId` без дефисов, `Debit→D`/`Credit→C`, строка 61 без `settlement_date_time`, отдельный файл на каждый `correlation_id`, `correlation_id` в имени файла, флаг `pcr_request.is_exported` и пропуск повторного экспорта, фильтры `--correlation-id`/`--date`, пустой случай. |
...
| `tests/functional/ImportBatchApiCest.php` | `/import-batch/load` и `/import-batch/list`: ручной запуск загрузки PH_TDS (merge переносит данные, снимает блокировку), запрет запуска занятой пачки (`is_processing`), фильтрация списка по секции (NRE не видит SUSPENSE_POSTING). |
