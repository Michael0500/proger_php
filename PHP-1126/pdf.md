...
Чтобы исправить проблему с кодировкой кириллицы в PDF, созданных через FPDI/TCPDF, выполните следующие шаги:

---

### 1. Укажите Unicode-шрифт
TCPDF и FPDI требуют явного указания шрифта, поддерживающего кириллицу. Например:
```php
// Устанавливаем шрифт с поддержкой кириллицы (например, freeserif)
...
Убедитесь, что текст передается в кодировке **UTF-8**:
```php
$userName = iconv('Windows-1251', 'UTF-8', 'Иван Иванов'); // Если исходные данные в CP1251
$pdf->Write(0, 'Пользователь: ' . $userName);
```
...
$pdf->SetXY(50, 100);
$pdf->Write(0, 'Пользователь: Иван Иванов');
...
### 4. Добавление кастомного шрифта
Если встроенных шрифтов недостаточно, добавьте свой (например, из файлов `.ttf`):
```php
// Добавляем шрифт (один раз, например, в начале скрипта)
$pdf->AddFont('DejaVuSans', '', 'dejavusans.php'); // Файл генерируется через TCPDF

// Используем шрифт
$pdf->SetFont('DejaVuSans', '', 12);
```

**Как сгенерировать файл шрифта:**
1. Скачайте шрифт (например, [DejaVuSans](https://dejavu-fonts.github.io/)).
2. Используйте утилиту `tcpdf_addfont` из TCPDF:
   ```bash
   php vendor/tecnickcom/tcpdf/tools/tcpdf_addfont.php -i DejaVuSans.ttf
   ```
3. Подключите сгенерированный файл (например, `dejavusans.php`).

---

### 5. Если используется FPDF
FPDF не поддерживает Unicode по умолчанию. Используйте:
- Расширение **UFPDF** (Unicode-версия FPDF):
  ```php
  require_once 'UFPDF/ufpdf.php';
  $pdf = new UFPDF();
  $pdf->AddFont('ArialUnicode', '', 'arialuni.php'); // Шрифт с кириллицей
  ```
- Или переключитесь на TCPDF.

---

### Важно:
- Всегда проверяйте кодировку исходного текста (используйте `mb_detect_encoding()`).
- Убедитесь, что шрифт содержит кириллические символы (например, `freeserif`, `dejavusans`).

Если проблема осталась:
1. Проверьте, что шрифт действительно поддерживает кириллицу.
2. Убедитесь, что текст передается в UTF-8.
3. Попробуйте другой шрифт (например, `cid0jp` для японских иероглифов, если нужна универсальность).